...
export default function CampaignGenerator({ 
  ...
  const [form, setForm] = useState({ item: '', price: '', cat: 'Fashion', disc: 15, otherDetails: '' });
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <div>
          ...
          <input className="w-full bg-gray-50 p-3 rounded-xl outline-none" 
            type="number" placeholder="15" value={form.disc} onChange={e => setForm({...form, disc: Number(e.target.value)})} />
        </div>
        <div>
        ...
        <div className="md:col-span-4">
          <label className="text-[10px] font-bold text-gray-400 uppercase mb-2 block">Other Details</label>
          <textarea className="w-full bg-gray-50 p-3 rounded-xl outline-none" 
            placeholder="Additional notes..." value={form.otherDetails} onChange={e => setForm({...form, otherDetails: e.target.value})} />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {campaigns.map((camp, i) => (
          <div key={i} className="bg-white p-6 rounded-[2rem] border-t-4 border-indigo-500 shadow-sm">
             ...
             <button
                className="w-full bg-[#0F172A] text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-black"
                onClick={async () => {
